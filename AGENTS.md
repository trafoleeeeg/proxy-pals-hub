...
## Как не конфликтовать (несколько агентов сразу)

1. **Ветка на задачу:** `agent/<claude|codex|astra>/<короткое-имя>`. Прямые коммиты
   в `main` запрещены — только Pull Request с зелёной проверкой `.github/workflows/ci.yml`.
2. **Никогда не переписывать отправленную историю:** без `push --force`, `rebase`,
   `amend`, `squash` по опубликованным коммитам — это ломает историю на стороне Lovable.
3. **Перед началом:** `git pull --ff-only origin main`. Перед PR — свежий `main`.
4. **Версию `desktop/package.json` меняет только тот, кто выпускает релиз.** Не поднимать
   её «заодно» — это вечный конфликт слияния.
5. **Одновременно один агент — один слой.** Договорённость по зонам:
   `src/routes/_authenticated/*` (панель), `src/lib/*.functions.ts` (сервер),
   `desktop/*` (клиент). Не править один и тот же файл параллельно.
6. **Миграции — только новые файлы** в `supabase/migrations/` с новой меткой времени.
   Существующие миграции не редактировать.
7. **Lovable тоже пишет в `main`.** Если PR конфликтует — подтянуть `main` и решить
   конфликт в своей ветке, а не в `main`.

## Доступ к данным по API

Агенты не получают доступ к базе напрямую. Есть ключи (`agent_keys`) и адреса
`/api/public/agent/*` — см. `agents/README.md`. Пароли прокси и cookies профилей
через API не отдаются никогда. Управление командой агентам не выдаётся.
