ALTER TABLE public.profile_browser_settings
  ADD COLUMN active_proxy_id uuid REFERENCES public.proxies(id) ON DELETE SET NULL,
  ADD COLUMN proxy_failover boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  END IF;
  IF NEW.active_proxy_id IS NOT NULL AND NOT EXISTS (
       SELECT 1 FROM public.proxies p
       JOIN public.browser_profiles b ON b.id = NEW.profile_id
       WHERE p.id = NEW.active_proxy_id AND p.team_id = b.team_id
     ) THEN
    RAISE EXCEPTION 'Invalid proxy selection' USING errcode = '22023';
  END IF;
