...
function renderer() {
  ...
  const closePopovers = (except) => {
    ...
    if (!except || except === manager) void run({ action: "chrome-overlay-height", value: 0 });
  };
  const syncPopoverLayer = (popover) => {
    if (!popover || popover.hidden || popover === manager) { void run({ action: "chrome-overlay-height", value: 0 }); return; }
