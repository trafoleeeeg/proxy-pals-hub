      ...
      <div className="scroll-thin min-w-0 overflow-x-auto"><Table className="min-w-max table-fixed text-xs"><TableHeader className="sticky top-0 z-10 bg-background"><TableRow>
        ...
        {rows.map((profile) => {
          ...
          return <ContextMenu key={profile.id}><ContextMenuTrigger asChild><TableRow data-state={selected.includes(profile.id) ? "selected" : undefined} draggable={canEdit && !locked(profile.id)} onDragStart={(event) => { event.dataTransfer.setData("application/x-umbra-profile", profile.id); event.dataTransfer.effectAllowed = "move"; }}>
            ...
            <ContextMenuItem disabled={!runtime.available || !runtime.ready || locked(profile.id)} onSelect={() => void runtime.start(profile.id).catch((error: Error) => toast.error(error.message))}><Play className="size-4" />Открыть</ContextMenuItem>
