...
function renderer() {
  const privacyCapabilities = ["gpu", "canvas", "audio", "fonts", "workers"];
  ...
  byId("privacy-save").addEventListener("click", async () => {
    ...
    const permissions = privacyCapabilities.filter((capability) => byId(`privacy-${capability}`).checked);
    const result = await run({ action: "set-site-privacy", origin: privacyConsent.origin, tabId: privacyConsent.tabId, permissions });
 ...
 *{box-sizing:border-box;letter-spacing:0}html{--chrome-height:90px}body{margin:0;background:var(--background);color:var(--foreground);font:13px "Segoe UI Variable","Segoe UI",system-ui,sans-serif;overflow:hidden}button,input{font:inherit;color:inherit}button{border:0;background:transparent;cursor:pointer}button:disabled{opacity:.35;cursor:default}button:focus-visible,input:focus-visible{outline:2px solid var(--ring);outline-offset:1px}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}#chrome{position:relative;z-index:5;background:var(--sidebar);box-shadow:0 1px 0 var(--border)}
 ...
 #privacy-popover{width:min(440px,calc(100vw - 20px));max-height:calc(100vh - 110px);overflow-y:auto;line-height:1.5}#privacy-popover p{margin:8px 4px}#privacy-popover label{display:flex;align-items:center;gap:9px;padding:5px 4px;cursor:pointer}#privacy-popover input{width:auto;height:auto}#privacy-origin{overflow-wrap:anywhere;font-weight:600}#privacy-notice{padding:5px 12px;font-size:11px;color:var(--muted-foreground);border-top:1px solid var(--border)}#privacy-button{font-size:20px}
