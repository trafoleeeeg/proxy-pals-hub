...
function renderer() {
  ...
  api.subscribe((incoming) => {
    ...
    if (visibleActiveId && state.activeId !== visibleActiveId) {
      for (const popover of [bookmarkPopover, extensionsPopover, menu]) popover.hidden = true;
    }
    ...
    list.replaceChildren(...extensions.map((extension) => {
      ...
      const meta = document.createElement("small");
      meta.textContent = extension.failed ? `Версия ${extension.version} · не загрузилось` : `Версия ${extension.version}`;
