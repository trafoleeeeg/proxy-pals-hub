...
export function TrashPage() {
  ...
  const canManageTrash = ws?.scope === "owner";
  const qc = useQueryClient();
  const listFn = useServerFn(listTrash);
  const restoreFn = useServerFn(restoreProfile);
  const [now, setNow] = useState(Date.now());
  useEffect(() => { const timer = setInterval(() => setNow(Date.now()), 60_000); return () => clearInterval(timer); }, []);
  const trash = useQuery({ queryKey: ["trash", ws?.teamId, ws?.userId], queryFn: () => listFn({ data: { teamId: ws!.teamId } }), enabled: !!ws && canManageTrash });
  ...
  if (!ws) return <p role="status">Загрузка рабочего пространства…</p>;
  if (!canManageTrash) return <Navigate to="/app" replace />;
