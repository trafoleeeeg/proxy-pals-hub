...
drop trigger if exists sync_assigned_profile_proxy on public.browser_profiles;
...
create table if not exists public.team_bookmark_defaults (
...
drop policy if exists "team bookmark defaults read" on public.team_bookmark_defaults;
...
drop trigger if exists team_bookmark_defaults_guard on public.team_bookmark_defaults;
