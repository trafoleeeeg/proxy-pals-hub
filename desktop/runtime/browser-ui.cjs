...
function renderer() {
  ...
  for (const action of ["back", "forward", "reload", "new", "close-profile"]) {
    document.getElementById(action).addEventListener("click", () => run({ action }));
  }
  ...
  api.subscribe((state) => {
    ...
    reload.title = current?.loading ? "Stop" : "Reload";
    ...
    tabs.replaceChildren(...state.tabs.map((tab) => {
      ...
      select.textContent = tab.title || "New tab"; select.title = tab.title || tab.url;
      select.onclick = () => run({ action: "select", id: tab.id });
      const close = document.createElement("button"); close.textContent = "\u00d7";
      close.className = "tab-close"; close.title = "Close tab"; close.setAttribute("aria-label", "Close tab");
...
</style></head><body><div id="top"><span id="profile">Umbra</span><div id="tabs" role="tablist" aria-label="Tabs"></div><button id="new" class="icon" title="New tab" aria-label="New tab">+</button><button id="close-profile" class="icon" title="Close profile" aria-label="Close profile">&#9211;</button></div>
<form id="navigate"><button id="back" type="button" class="icon" title="Back" aria-label="Back">&#8592;</button><button id="forward" type="button" class="icon" title="Forward" aria-label="Forward">&#8594;</button><button id="reload" type="button" class="icon" title="Reload" aria-label="Reload">&#8635;</button><input id="address" type="text" aria-label="Address" placeholder="Search or enter address" autocomplete="off" spellcheck="false"><button class="icon" type="submit" title="Go" aria-label="Go">&#8594;</button></form>
<div id="error" role="status" aria-live="polite"></div><script>${script}</script></body></html>`;
