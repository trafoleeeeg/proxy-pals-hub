...
function renderer() {
  ...
  api.subscribe((state) => {
    ...
    bookmarksBar.replaceChildren(...(state.bookmarks || []).map((bookmark) => {
      ...
      button.innerHTML = icon("globe") + `<span></span>`; button.querySelector("span").textContent = bookmark.title || new URL(bookmark.url).hostname;
