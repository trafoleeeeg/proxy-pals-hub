...
</style></head><body><div id="top"><span id="profile">Umbra</span><div id="tabs" role="tablist" aria-label="Tabs"></div><button id="new" class="icon" title="Новая вкладка (Ctrl+T)" aria-label="Новая вкладка">+</button><button id="close-profile" class="icon" title="Закрыть профиль" aria-label="Закрыть профиль">&#9211;</button></div>
