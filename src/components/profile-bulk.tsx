...
export function ProfileBulkDialog({ action, ids, teamId, isOwner, blocked, proxies, folders = [], onClose, onSaved }: {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !busy) onClose(); }}><DialogContent role={action === "delete" ? "alertdialog" : "dialog"} className="max-h-[90dvh] w-[calc(100%-2rem)] overflow-y-auto sm:max-w-xl">
    <DialogHeader className="min-w-0 pr-5"><DialogTitle className="break-words leading-snug tracking-normal">{titles[action]}</DialogTitle><DialogDescription>{action === "delete" ? `Профили (${ids.length}) попадут в корзину на 14 дней. Их можно восстановить вместе с cookies и настройками.` : `Выбрано профилей: ${ids.length}`}</DialogDescription></DialogHeader>
